<div class="row">
    <div class="col-12">
        <div class="card-box table-responsive">
            <h4 class="m-t-0 header-title">Approve Cabut Blokir Nasabah</h4>

            <div class="row">
                <div class="col-lg-4">
                    <div class="form-group">
                        <label for="document">Berita Acara</label>
                        <input type="text" value="<?= $data_blokir->berita_acara_unblock ?>" name="document" parsley-trigger="change" class="form-control" id="document" disabled>
                    </div>
                    <div class="form-group">
                        <label for="screenshot">Screenshot Bukti</label>
                        <input type="text" value="<?= $data_blokir->screenshot_unblock ?>" name="screenshot" parsley-trigger="change" class="form-control" id="screenshot" disabled>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group text-right m-b-0">
                        <button class="btn btn-primary waves-effect waves-light" onclick="action('Unblock Rejected', 'Unblock Rejected')">
                            Reject
                        </button>
                        <button class="btn btn-primary waves-effect waves-light" onclick="action('Unblocked', 'Unblocked Approved')">
                            Approve
                        </button>
                    </div>
                </div>
...
<script>
    function action(statusRequest, swalTittle) {
        $.post("<?= base_url("/unblock-approve-input/" . $data_blokir->id) ?>", {
                ...
                if (data.success === true) {
                    Swal.fire({
                        ...
                        if (response.isConfirmed) {
                            window.location.href = "<?= base_url("unblock-approve-list") ?>";
                        }
                    });
                } else {
                    Swal.fire({
